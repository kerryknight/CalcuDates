...
#define RxDLogDebug(...) NSLog(@"%s %@", __PRETTY_FUNCTION__, [NSString stringWithFormat:__VA_ARGS__])
#else
#define RxDLogDebug(...) do { } while (0)
#endif

#if defined (DEBUG_LOG) && (DEBUG_LOG >= DEBUG_LOGLEVEL_INFO)
#define RxDLogInfo(...) NSLog(@"INFO: %s %@", __PRETTY_FUNCTION__, [NSString stringWithFormat:__VA_ARGS__])
#else
#define RxDLogInfo(...) do { } while (0)
#endif

#if defined (DEBUG_LOG) && (DEBUG_LOG >= DEBUG_LOGLEVEL_WARN)
#define RxDLogWarn(...) NSLog(@"WARNING: %s %@", __PRETTY_FUNCTION__, [NSString stringWithFormat:__VA_ARGS__])
#else
#define RxDLogWarn(...) do { } while (0)
#endif

#if !defined (DEBUG_LOG) || (DEBUG_LOG >= DEBUG_LOGLEVEL_ERROR)
#define RxDLogError(...) NSLog(@"ERROR: %s %@", __PRETTY_FUNCTION__, [NSString stringWithFormat:__VA_ARGS__])
#else
#define RxDLogError(...) do { } while (0)
#endif

#define RxDLog(...) NSLog(@"%s %@", __PRETTY_FUNCTION__, [NSString stringWithFormat:__VA_ARGS__])
