...
- (instancetype)init {
    self = [super init];
    RxDLogInfo(@"create: %p", (__bridge void*)self);
    return self;
}

- (void) dealloc {
    RxDLogInfo(@"dealloc: %p", (__bridge void*)self);
    if (_handler_queue) {
        RxDLogWarn(@"handlers not signaled");
...
- (void) synced_cancelWithReason:(id)reason {
    ...
    if (_state == Pending) {
        RxDLogDebug(@"cancelled %p.", (__bridge void*)(self));
        _result = reason;
        _state = Cancelled;
        if (_handler_queue) {
            dispatch_resume(_handler_queue);
    ...
    if (_state != Cancelled) {
        ...
        while (range.first != range.second) {
            RxDLogDebug(@"%p forwarding cancel to %p", key, (__bridge void*)((*(range.first)).second));
                     ...
                     returnPromise:(BOOL)returnPromise    __attribute((ns_returns_retained))
{
    ...
    RxDLogInfo(@"promise %p register handlers %p %p running on %s returning promise: %p",
        ...
        @autoreleasepool { // mainly for releasing unused return values from the handlers
            ...
            if (strongReturnedPromise) {
                ...
                if (ps.state == Cancelled) {
                    ...
                    RxDLogInfo(@"%p add child %p", (__bridge void*)(blockSelf), (__bridge void*)(strongReturnedPromise));
